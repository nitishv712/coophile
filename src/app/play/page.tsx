...
type LibraryState = "idle" | "loading" | "missing" | "ready";

/** Centred message used by every non-playing state. */
function Notice({
  icon,
  title,
  body,
...
function PlayContent() {
  ...
  // Catalog games come from the server, so the record arrives async. The ROM
  // itself is streamed straight from its endpoint — no need to buffer it here.
  useEffect(() => {
    if (!gameId) return;
    let cancelled = false;
    ...
    fetchGame(gameId)
      .then((found) => {
        if (cancelled) return;
        setGame(found);
        setLibraryState(found.rom ? "ready" : "missing");
      })
      .catch(() => {
        if (!cancelled) setLibraryState("missing");
      });
  ...
  const requested = searchParams.get("system") as SystemType | null;
  const system = game?.system ?? (requested && SYSTEMS[requested] ? requested : null);
  const romUrl = gameId ? (game?.rom ? romEndpoint(gameId) : null) : droppedRomUrl;
  ...
  // Still pulling the record out of the database.
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="font-body text-sm text-on-surface-variant">
          Loading {game?.title ?? "game"}…
        </p>
      </div>
    );
  }

  // Either the game is unknown, or an admin has not attached its ROM yet.
  if (gameId && libraryState === "missing") {
    return (
      <Notice
        icon={game ? "hourglass_empty" : "search_off"}
        title={game ? `No ROM for ${game.title}` : "Game not found"}
        body={
          game
            ? `${game.title} is in the catalog but has no ROM attached yet. An admin needs to upload one.`
