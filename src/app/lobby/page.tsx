...
function LobbyContent() {
  ...
  // Invite links land here as /lobby?room=ABC123 — derive the starting state
  // from the URL rather than syncing it in an effect.
  const invitedRoom = searchParams.get("room")?.toUpperCase() ?? "";
  ...
  // Announce what we are running once the channel is open.
  //
  // Waits for the game record to arrive first: announcing a null hash while the
  // fetch is still in flight makes the other side flash a spurious "one of you
  // has no ROM" before the corrected hello lands.
  useEffect(() => {
    if (!state.channelOpen) return;
    if (gameId && !game) return;
    session.send({ t: "hello", gameId: gameId ?? null, romHash: localHash });
  }, [state.channelOpen, session, gameId, game, localHash]);
   ...
   * host is browsing on localhost, so ask the server what address it can
   * actually be reached on and prefer that.
   */
  ...
  useEffect(() => {
    ...
    fetch("/api/network")
      ...
      .then((data: { origins?: string[] }) => {
        if (!cancelled) setShareOrigin(data.origins?.[0] ?? null);
      })
      .catch(() => {
        if (!cancelled) setShareOrigin(null);
      });
    return () => {
      cancelled = true;
    };
            ...
            {state.roomCode && state.isHost && (
              <div className="card p-8 flex flex-col gap-5 relative overflow-hidden">
                ...
                {copyFailed && (
                  <p
                    id="copy-failed-note"
                    className="font-body text-xs text-tertiary -mt-2"
                  >
              ...
              <div className="card overflow-hidden flex flex-col h-full shadow-sm">
                ...
                {state.channelOpen && (
                  <>
                    <div
                      id="chat-log"
                      className="px-6 pb-2 h-44 overflow-y-auto space-y-2"
                    >
                      {chat.length === 0 && (
